...
def get_check_date(check_date_offset=1):
    session = get_session()
    query_check_date = f"""SELECT CURRENT_DATE - {check_date_offset} AS check_date"""
...
def get_check_date_offset(check_date):
    session = get_session()
    query_check_date_offset = f"""SELECT CURRENT_DATE - DATE '{check_date}' AS check_date_offset """
...
def get_flag(metric_name, date_offset=1):
    session = get_session()
    query_flag = f"""SELECT mavg_7d_change, mavg_7d_change_II, accu_trend
                          FROM pp_oap_xinglv1_t.compass_case_monitoring
                         WHERE "type" = '{metric_name}'
                               AND datetime = CURRENT_DATE - {date_offset}"""
...
def get_data(metric_name, date_offset=1):
    ...
    query_data = f"""SELECT *
                          FROM pp_oap_xinglv1_t.compass_case_monitoring
                         WHERE "type" = '{metric_name}'
                               AND datetime BETWEEN -90 - {date_offset} AND CURRENT_DATE - {date_offset}
                         ORDER BY datetime"""
...
def get_driver(trend_type, trend_direction, date_offset=1):
    ...
    if trend_type == "mavg_7d":
        if trend_direction == 1:
            query_driver = f"""SELECT "name", "type", mavg_7d_change_rate, accu_trend, positive_negative_backlog
                                           ...
                                           WHERE mavg7d_driver_share > 0
                                                 AND datetime = CURRENT_DATE - {date_offset}
                                         QUALIFY ROW_NUMBER() OVER (PARTITION BY datetime ORDER BY mavg7d_driver_share desc) <= 3"""
            ...
            query_driver = f"""SELECT "name", "type", mavg_7d_change_rate, accu_trend, positive_negative_backlog
                                           ...
                                           WHERE mavg7d_driver_share < 0
                                                 AND datetime = CURRENT_DATE - {date_offset}
                                         QUALIFY ROW_NUMBER() OVER (PARTITION BY datetime ORDER BY mavg7d_driver_share) <= 3"""
        ...
        if trend_direction == 1:
            query_driver = f"""SELECT "name", "type", mavg_7d_change_rate, accu_trend, positive_negative_backlog
                                           ...
                                           WHERE accutrend_driver_share > 0
                                                 AND datetime = CURRENT_DATE - {date_offset}
                                         QUALIFY ROW_NUMBER() OVER (PARTITION BY datetime ORDER BY accutrend_driver_share desc) <= 3"""
        elif trend_direction == -1:
            query_driver = f"""SELECT "name", "type", mavg_7d_change_rate, accu_trend, positive_negative_backlog
                                                ,positive_negative_backlog*accu_trend AS accutrend_driver_share
                                                ,positive_negative_backlog*mavg_7d_change_rate AS mavg7d_driver_share
                                           ...
                                           WHERE accutrend_driver_share < 0
                                                 AND datetime = CURRENT_DATE - {date_offset}
                                         QUALIFY ROW_NUMBER() OVER (PARTITION BY datetime ORDER BY accutrend_driver_share) <= 3"""
...
def draw_mavg_chart(metric_name, chart_data):
    ...
    plt.plot(
        chart_data.datetime, chart_data.value, label=metric_name, color="steelblue", linewidth=2
    )
    plt.plot(
        chart_data.datetime,
        chart_data.mavg_7d,
    ...
    plt.plot(
        chart_data.datetime,
        chart_data.mavg_7d * 1.1,
        label="1.1 * Moving Average",
        color="dimgray",
        linestyle="--",
        linewidth=1,
    )
    plt.plot(
        chart_data.datetime,
        chart_data.mavg_7d * 0.9,
        label="0.9 * Moving Average",
        color="dimgray",
        linestyle="--",
        linewidth=1,
    )
    ...
    for a, b, c in zip(
        chart_data.datetime[-35:], chart_data.value[-35:], chart_data.mdiff_2d_change[-35:]
    ):
        c_color = "grey"
        if c > 0:
            c_color = "green"
...
if __name__ == "__main__":
    ...
    ##### Backlog #####
    BacklogVol_flag = get_flag(
        "PROJ:Compass Monitoring|CAT:BacklogVol", check_date_offset
    )  # Backlog monitor result flag
    BacklogVol_data = get_data(
        "PROJ:Compass Monitoring|CAT:BacklogVol", check_date_offset
    )  # Backlog data
    ...
    if BacklogVol_flag["mavg_7d_change"][0] != 0:  # (> 1.1*benchmark OR < 0.9*benchmark)
        ...
        if BacklogVol_flag["mavg_7d_change"][0] == 1:  # Increase
            ...
            html_text = (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Compass Case Backlog Volume Increasd more than 10% of last 7 days average!</p>'
                "<br>"
            )
            ...
            html_text = (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Compass Case Backlog Volume Decreased more than 10% of last 7 days average!</p>'
                "<br>"
            )
            driver = get_driver("mavg_7d", -1, check_date_offset)
        ...
        abs(BacklogVol_flag["accu_trend"][0]) == 5 and BacklogVol_flag["mavg_7d_change_II"][0] != 0
    ):  # (continued 5 days up/down trend) AND (> 1.1*benchmark OR < 0.9*benchmark)

        if BacklogVol_flag["accu_trend"][0] == 5:  # Increase
            subject = "Alert! Compass Case Backlog Volume increased! " + check_date
            html_text = (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Compass Case Backlog Volume kept Increasing for more than 5 days!</p>'
                "<br>"
            )
            ...
            subject = "Alert! Compass Case Backlog Volume decreased! " + check_date
            html_text = (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Compass Case Backlog Volume kept Decreasing for more than 5 days!</p>'
                "<br>"
            )
            driver = get_driver("accu_trend", -1, check_date_offset)
    ...
    html_text += (
        '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">The top drivers are: </p>'
        "<br>"
    )
    ...
    for index, row in driver.iterrows():
        ...
        html_text += (
            '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Top '
            + str(index + 1)
            + ": "
            + row["name"]
            + "</p>"
        ...
        if row["accu_trend"] > 0:
            html_text += (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">kept Increasing for '
                + str(row["accu_trend"])
                + " days </p>"
                "<br>"
            )
            ...
            html_text += (
                '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">kept Dncreasing for '
                + str(abs(row["accu_trend"]))
                + "days </p>"
                "<br>"
            )
        html_text += (
            '<p style = "font-family:Arial;font-size:14px;font-weight:bold;color:red">Change Rate is '
            + str(row["mavg_7d_change_rate"])
            + "</p>"
            "<br>"
        )
        html_text += (
            "<img src=" + row["name"].replace(" ", "") + '.png width="2400" height="900" />'
        )
