...
from sqlalchemy.orm import DeclarativeBase, Mapped, Session, aliased, mapped_column, relationship
...
def select_subquery(engine: Engine) -> None:
    ...
    stmt = (
        select(User, address_subq).join_from(User, address_subq).order_by(User.id, address_subq.id)
    )
    """
