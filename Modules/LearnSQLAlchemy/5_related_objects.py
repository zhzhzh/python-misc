...
def using_relationships_in_query(engine: Engine) -> None:
    ...
    print(
        select(Address.email_address).select_from(User).join(User.addresses)
    )  # this is join with relationship()
    """
    ...
    )  # this is due to the ForeignKeyConstraint
...
def raise_load(engine: Engine) -> None:
    with Session(engine) as session:
        ...
        u1 = session.execute(select(User).options(selectinload(User.addresses))).scalars().first()
